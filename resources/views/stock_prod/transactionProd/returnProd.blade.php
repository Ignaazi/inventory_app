    ...
    <div class="mb-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 font-nunito">
        <div>
            <h2 class="text-xl md:text-2xl font-black text-slate-900 dark:text-white tracking-tight uppercase">
                Production Stock Return Activities
            </h2>
            <p class="text-[11px] md:text-[13px] font-bold text-slate-500 dark:text-slate-400">Track and manage returned components from production line back to stock</p>
        </div>

        {{-- Tombol Scan RETURN (Orange Gradient, Tanpa Icon) --}}
        <div class="flex items-center gap-2 w-full sm:w-auto">
            <a href="{{ Route::has('prod.transaction.return.scan') ? route('prod.transaction.return.scan') : url('/prod/transaction/return/scan') }}" 
    ...
    <div class="w-full overflow-hidden rounded-xl border border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900 pt-4 shadow-sm">
        ...
        <div class="mb-4 flex flex-col gap-3 px-5 sm:flex-row sm:items-center sm:justify-between font-nunito">
            ...
            <div class="grid grid-cols-12 gap-2 w-full sm:w-auto order-1 sm:order-2">
                ...
                {{-- TOMBOL EXPORT CSV --}}
                <button type="button" onclick="exportTableToCSV('production-stock-return.csv')" class="col-span-4 flex items-center justify-center gap-1.5 rounded-lg border border-gray-300 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-2 text-xs md:text-[13px] font-black text-slate-950 dark:text-white shadow-sm hover:bg-slate-50 dark:hover:bg-slate-700 transition-all active:scale-95 cursor-pointer font-nunito">
        ...
        {{-- AREA SCROLL TABEL BLUE STOCK RETURN --}}
