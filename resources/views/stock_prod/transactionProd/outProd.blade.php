...
<div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
  ...
  <div class="flex flex-col gap-2 mb-6 sm:flex-row sm:items-center sm:justify-between">
    <div>
      ...
      <p class="text-xs font-medium text-slate-600 dark:text-gray-400">Track and manage discharged nozzles from the production floor</p>
    </div>

    <div class="flex items-center gap-3">
      <a href="#"
        class="inline-flex items-center gap-2 rounded-lg border border-transparent bg-gradient-to-r from-rose-600 to-orange-500 px-3 py-2 text-xs font-bold text-white shadow-sm hover:opacity-90 transition-all active:scale-95"
      >
        <i class="fas fa-qrcode"></i> Scan OUT (Prod)
      </a>
      ...
      >
        <i class="fas fa-keyboard"></i> Manual OUT (Prod)
      </a>
    </div>
  </div>
      ...
      <table class="min-w-full text-left border-collapse" id="history-table">
        <thead>
          <tr class="border-gray-100 border-y dark:border-gray-800 bg-gray-50/50 dark:bg-slate-800/40">
            <th class="py-3 px-3 text-xs font-bold text-slate-950 uppercase dark:text-white text-center w-12">NO</th>
            <th class="py-3 px-3 text-xs font-bold text-slate-950 uppercase dark:text-white text-center">OUTPRODUCTION ID</th>
            <th class="py-3 px-3 text-xs font-bold text-slate-950 uppercase dark:text-white text-center">INPRODUCTION ID</th>
            <th class="py-3 px-3 text-xs font-bold text-slate-950 uppercase dark:text-white">DATE</th>
            <th class="py-3 px-3 text-xs font-bold text-slate-950 uppercase dark:text-white">NIK</th>
            <th class="py-3 px-3 text-xs font-bold text-slate-950 uppercase dark:text-white">LINE ORIGIN</th>
            <th class="py-3 px-3 text-xs font-bold text-slate-950 uppercase dark:text-white">NO NOZZLE</th>
            <th class="py-3 px-3 text-xs font-bold text-slate-950 uppercase dark:text-white">TRANSACTION OUT ID</th>
            <th class="py-3 px-3 text-xs font-bold text-slate-950 uppercase dark:text-white">REQUEST NO</th>
            <th class="py-3 px-3 text-xs font-bold text-slate-950 uppercase dark:text-white">BARCODE ID</th>
            <th class="py-3 px-3 text-xs font-bold text-slate-950 uppercase dark:text-white">STOCK PROD ID</th>
            <th class="py-3 px-3 text-xs font-bold text-slate-950 uppercase dark:text-white text-center">QTY OUT</th>
            <th class="py-3 px-3 text-xs font-bold text-slate-950 uppercase dark:text-white text-center">STATUS</th>
            <th class="py-3 px-3 text-xs font-bold text-slate-950 uppercase dark:text-white text-center">REMARK</th>
            <th class="py-3 px-3 text-xs font-bold text-slate-950 uppercase dark:text-white">COMMENT</th>
            <th class="py-3 px-3 text-xs font-bold text-slate-950 uppercase dark:text-white">CREATED AT</th>
          </tr>
        </thead>
        <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
          ...
          @if(isset($history) && count($history) > 0)
            ...
            <tr class="table-row-item hover:bg-gray-50/50 transition-colors duration-200 dark:hover:bg-white/[0.02]">
              ...
              <td class="py-3 px-3 text-xs font-bold text-slate-950 dark:text-white text-center font-mono">
                {{ (method_exists($history, 'currentPage')) ? ($history->currentPage() - 1) * $history->perPage() + $loop->iteration : $loop->iteration }}
              </td>
              ...
              {{-- Kolom Pelacakan inproduction_id (Senjata buat jawab pertanyaan Dosen) --}}
              <td class="py-3 px-3 text-xs font-bold text-slate-950 dark:text-white text-center font-mono bg-slate-50/40 dark:bg-slate-800/20">
                {{ $log->inproduction_id ?? '-' }}
              </td>
              
              <td class="py-3 px-3 text-xs font-semibold text-slate-950 dark:text-white whitespace-nowrap">
              ...
              {{-- Bungkusan kotak Line Origin --}}
              <td class="py-3 px-3 whitespace-nowrap">
                <span class="inline-flex items-center justify-center rounded border border-gray-300 bg-gray-100 px-2.5 py-1 text-xs font-bold text-slate-950 dark:border-slate-700 dark:bg-slate-800 dark:text-white">
              ...
              {{-- Bungkusan kotak Request No --}}
              <td class="py-3 px-3 whitespace-nowrap">
                <span class="inline-flex items-center justify-center rounded border border-amber-300 bg-amber-50 px-2.5 py-1 text-xs font-bold text-amber-900 font-mono dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-400">
                  {{ $log->request_no ?? '-' }}
                </span>
              </td>
              ...
              {{-- Bungkusan kotak Qty OUT (Warna Rose/Merah tanda barang keluar) --}}
              <td class="py-3 px-3 text-center">
                <span class="inline-flex items-center justify-center rounded border border-rose-300 bg-rose-50 px-3 py-1 text-xs font-bold text-rose-900 dark:border-rose-500/30 dark:bg-rose-500/10 dark:text-rose-400">
                  -{{ $log->qty_out ?? 0 }}
                </span>
              </td>
              
              {{-- Bungkusan kotak Status --}}
              <td class="py-3 px-3 text-center">
                <span class="status-cell inline-flex items-center justify-center rounded border px-3 py-1 text-xs font-bold uppercase tracking-wide
                  @if(strtolower($log->status ?? '') == 'success') border-emerald-300 bg-emerald-50 text-emerald-900 dark:border-emerald-500/30 dark:bg-emerald-500/10 dark:text-emerald-400
                  @elseif(strtolower($log->status ?? '') == 'pending') border-orange-300 bg-orange-50 text-orange-900 dark:border-orange-500/30 dark:bg-orange-500/10 dark:text-orange-400
                  @else border-rose-300 bg-rose-50 text-rose-900 dark:border-rose-500/30 dark:bg-rose-500/10 dark:text-rose-400 @endif">
                  {{ $log->status ?? 'success' }}
                </span>
              </td>
              
              {{-- Bungkusan kotak Remark --}}
              <td class="py-3 px-3 text-center whitespace-nowrap">
                ...
                <span class="remark-cell inline-flex items-center justify-center rounded border px-3 py-1 text-xs font-bold uppercase tracking-wide
                  @if($isManual) border-blue-300 bg-blue-50 text-blue-900 dark:border-blue-500/30 dark:bg-blue-500/10 dark:text-blue-400
                  @elseif($isScan) border-purple-300 bg-purple-50 text-purple-900 dark:border-purple-500/30 dark:bg-purple-500/10 dark:text-purple-400
                  @else border-gray-300 bg-gray-50 text-slate-900 dark:border-slate-700 dark:bg-slate-800 dark:text-white @endif">
                  {{ $remarkText ? $log->remark : '-' }} 
                </span>
              </td>
              
              <td class="py-3 px-3 text-xs font-semibold text-slate-700 dark:text-gray-300 max-w-[150px] truncate" title="{{ $log->comment }}">
                {{ $log->comment ?? '-' }}
              </td>

              <td class="py-3 px-3 text-xs font-semibold text-slate-950 dark:text-slate-400 whitespace-nowrap font-mono">
                {{ $log->created_at ?? '-' }}
              </td>
            </tr>
            @endforeach
          ...
          @else
            <tr>
              <td colspan="16" class="py-12 text-center text-slate-950 dark:text-white italic bg-white dark:bg-slate-900 text-xs font-bold">
