...
<div class="mx-auto w-full max-w-7xl pb-12 px-4 sm:px-6 font-nunito text-black dark:text-white" x-data="materialReceivedApproveHandler()" x-cloak>
    
    <!-- HEADER SECTION -->
    <div class="mb-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 font-nunito print:hidden">
        <div>
            <h2 class="text-xl md:text-2xl font-black text-black dark:text-white tracking-tight">APPROVE MATERIAL RECEIVED</h2>
            <p class="text-[11px] md:text-[13px] font-bold text-slate-600 dark:text-slate-400">Engineering Final Authorization Desk</p>
        </div>
    ...
    <div class="bg-white dark:bg-boxdark border border-slate-300 dark:border-strokedark rounded-md shadow-sm overflow-hidden print:hidden mb-10">
        <form id="materialApproveForm" action="{{ route('engineering.spv.approve', $receiving->id) }}" method="POST" @submit.prevent="submitForm">
            @csrf
            
            <input type="hidden" name="approved_signature" x-bind:value="spvSignaturePathHidden">

            <div class="p-5 sm:p-6">
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    
                    <!-- LEFT SIDE: PARAMETER DATA INPUTS (READONLY EXCEPT SUPERVISOR NOTES) -->
                    <div class="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div class="flex flex-col gap-1.5">
                            ...
                            <input type="number" x-model="qty_received" readonly class="w-full rounded-md border border-slate-300 bg-slate-100 py-2 px-3 text-xs font-bold text-black cursor-not-allowed outline-none dark:bg-meta-4/30">
                        </div>

                        <!-- 7. Qty Gap -->
                        <div class="flex flex-col gap-1.5 sm:col-span-2">
                            <label class="text-xs font-black uppercase text-black tracking-wider">7. QTY GAP (REMAINING BALANCE)</label>
                            <div class="w-full rounded-md border py-2 px-3 text-xs font-black bg-slate-100 shadow-sm flex items-center justify-between border-slate-300 dark:bg-meta-4/30">
                                <span x-text="qty_gap == 0 ? '0 Pcs' : '-' + qty_gap + ' Pcs'">0 Pcs</span>
                                <span :class="qty_gap == 0 ? 'bg-emerald-600 text-white' : 'bg-amber-500 text-white'" 
                                      class="px-2.5 py-0.5 rounded text-[10px] font-black tracking-wider" 
                                      x-text="qty_gap == 0 ? 'CLOSED' : 'OPEN'">
                                </span>
                            </div>
                        </div>

                        <!-- Staff Remark / Condition -->
                        <div class="flex flex-col gap-1.5 sm:col-span-2">
                            <label class="text-xs font-black uppercase text-black tracking-wider">STAFF REMARK / CONDITION</label>
                    ...
                    <div class="flex flex-col justify-between bg-slate-50 dark:bg-slate-900/40 p-4 rounded-md border border-slate-300 dark:border-strokedark relative z-0">
                        <div>
                            ...
                            <div class="relative w-full h-28 bg-white dark:bg-slate-950 border border-slate-300 dark:border-gray-800 rounded-md flex items-center justify-center p-2 overflow-hidden shadow-inner">
                                <div class="absolute inset-0 z-10 flex items-center justify-center p-2" x-show="spvSignatureImg">
                                    <img :src="spvSignatureImg" class="max-h-full max-w-full object-contain mx-auto my-auto block">
                                </div>
                                
                                <div class="z-30 text-center" x-show="!spvSignatureImg">
                                    <div class="text-indigo-600 dark:text-indigo-400 font-mono text-[9px] uppercase tracking-wider border border-indigo-200 dark:border-indigo-900 bg-indigo-50 px-2.5 py-1.5 rounded-md">
                                        Secure Security Key<br>
                                        <span class="text-[8px] text-black font-sans font-bold tracking-normal" x-text="spv_name ? 'Linked to: ' + spv_name : 'Waiting...'"></span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex justify-between items-center text-[10px] font-black uppercase tracking-wider pt-2 border-t border-slate-300 dark:border-strokedark mt-3">
                            <span class="text-black dark:text-white">OTORISASI: 
                                <span :class="spvSignatureImg ? 'bg-emerald-600 text-white' : 'bg-slate-600 text-white'" class="ml-1 px-2.5 py-1 rounded text-[9px] font-black tracking-wide" x-text="spvSignatureImg ? 'READY TO APPROVE' : 'NONE'"></span>
                            </span>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-6 pt-4 border-t border-slate-200 dark:border-strokedark flex flex-col sm:flex-row justify-end gap-2.5">
                    ...
                    <button type="submit" class="w-full sm:w-auto bg-gradient-to-r from-emerald-600 via-emerald-500 to-teal-500 hover:opacity-90 text-white rounded-md px-4 py-1.5 text-xs font-black uppercase tracking-wider transition-all active:scale-95 border-none cursor-pointer shadow-md">
                        Approve MR Document
                    </button>
                </div>
            </div>
                ...
                <table class="w-full border-collapse text-xs border border-black">
                    <tbody>
                        ...
                        <tr class="border-b border-black">
                            ...
                            <td class="py-2.5 px-4 font-black text-black" x-text="qty_received ? qty_received + ' Pcs' : '0 Pcs'">0 Pcs</td>
                        </tr>
                        <tr class="border-b border-black">
            ...
            <div class="grid grid-cols-3 gap-0 border border-black text-center text-xs mt-8 rounded-sm overflow-hidden">
                ...
                <div class="flex flex-col justify-between h-36 bg-white relative z-0">
                    ...
                    <div class="relative flex items-center justify-center h-20 w-full bg-white overflow-hidden mx-auto">
                        ...
                        <div class="z-30 px-2 my-auto" x-show="spv_name && !spvSignatureImg">
                            <div class="text-emerald-600 font-mono text-[9px] font-black uppercase tracking-tighter border border-emerald-300 bg-emerald-50 py-0.5 rounded mx-auto max-w-[130px]">
                                APPROVAL STAGE
                            </div>
                        </div>
                    </div>
...
<script>
    function materialReceivedApproveHandler() {
        const currentSpvNik = "{{ auth()->check() ? auth()->user()->nik : '' }}";
        const currentSpvName = "{{ auth()->check() ? auth()->user()->name : '' }}";
        const currentSpvSignature = "{{ auth()->check() && auth()->user()->signature_path ? auth()->user()->signature_path : '' }}";
        
        return {
            receiving: {!! json_encode($receiving) !!},
            ...
            qty_received: parseInt("{{ $receiving->qty_received ?? 0 }}"),
            qty_gap: 0,
            remark: {!! json_encode($receiving->remark ?? '') !!},
            notes: '', 
            
            // Tanda Tangan Costing (Tahap 1)
            ...
            spvSignatureImg: currentSpvSignature ? (currentSpvSignature.startsWith('http') ? currentSpvSignature : "{{ asset('storage') }}/" + currentSpvSignature.replace(/^\/?(storage\/)?/, '')) : null,
            ...
            submitForm() {
                const form = document.getElementById('materialApproveForm');
                
                Swal.fire({
                    title: 'Berikan Approval Akhir?',
                    text: "Dokumen Material Received ini akan disetujui penuh (FULLY APPROVED) dan menutup sisa saldo balance.",
                    icon: 'warning',
