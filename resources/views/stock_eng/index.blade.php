...
<script>
    ...
    function openModal(mode, element = null) {
        ...
        if (mode === 'edit' && element) {
            ...
            // FIXED: Javascript sekarang memetakan kecocokan master list ke primary key (data.sparepart_id)
    ...
    function openRackModal() {
        document.getElementById('modalRack').classList.remove('hidden');
    }

    function closeRackModal() {
        document.getElementById('modalRack').classList.add('hidden');
    }

    function validateDuplicateStock(formElement) {
        const selectedRakId = document.getElementById('rak_id').value;
        const selectEl = document.getElementById('sparepart_id');
        const currentItemId = document.getElementById('current_item_id').value;

        // FIXED: Pencocokan client-side duplikasi menggunakan primary key database ($sp->id)
        const isDuplicate = currentInventoryData.some(item => {
            if (currentItemId && String(item.id) === String(currentItemId)) {
                return false; 
            }
            return String(item.rak_id) === String(selectedRakId) && String(item.sparepart_id) === String(selectEl.value);
        });
        ...
        if (isDuplicate) {
            Swal.fire({
                ...
                text: 'Gagal! Nozzle/Sparepart ini sudah terdaftar di rak tersebut. Silakan pilih kombinasi rak lain.',
    ...
    document.querySelectorAll('.btn-delete-rak').forEach(button => {
        button.addEventListener('click', function(e) {
            ...
            Swal.fire({
                title: 'Delete Rack?',
                text: "Make sure this rack has no items assigned inside it!",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#ef4444',
                cancelButtonColor: '#64748b',
                confirmButtonText: 'Yes, remove rack!'
            }).then((result) => {
                if (result.isConfirmed) {
                    form.submit();
                }
