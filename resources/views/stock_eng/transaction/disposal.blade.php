...
<div class="font-nunito w-full p-3 md:p-6 bg-slate-50/30 dark:bg-slate-950 min-h-screen transition-all duration-300">
    ...
    {{-- Alert Banner Top Counter (TEMA ROSE / RED FOR DISPOSAL) --}}
    <div class="mb-4 flex items-center gap-2 rounded-xl border border-rose-200 bg-rose-50 dark:bg-rose-950/30 dark:border-rose-900/50 px-3 py-2.5 md:px-4 md:py-3 shadow-sm">
        ...
        <p class="text-[12px] md:text-[14px] font-bold text-rose-800 dark:text-rose-400 font-nunito leading-tight">
            ...
            Total {{ (isset($history) && method_exists($history, 'total')) ? $history->total() : count($history ?? []) }} disposal & scrapped sparepart logs recorded.
        </p>
    </div>
    ...
    <div class="mb-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 font-nunito">
        <div>
            <h2 class="text-xl md:text-2xl font-black text-slate-900 dark:text-white tracking-tight uppercase flex items-center gap-2">
                <i class="fa-solid fa-trash-can text-rose-600 dark:text-rose-400"></i> Stock Disposal Activities
            </h2>
            <p class="text-[11px] md:text-[13px] font-bold text-slate-500 dark:text-slate-400">Track scrapped, damaged, or decommissioned engineering components</p>
        </div>

        <div class="flex items-center gap-2 w-full sm:w-auto">
            <a href="{{ route('stock_eng.transaction.disposal.scan') }}" 
               class="inline-flex items-center justify-center gap-1.5 h-9 rounded-lg bg-gradient-to-r from-rose-600 via-rose-500 to-red-600 px-3.5 text-[11px] font-black text-white shadow-md hover:opacity-90 tracking-wider uppercase active:scale-95 transition-all font-nunito w-full sm:w-auto text-center no-underline">
                <i class="fa-solid fa-qrcode text-xs"></i> Scan Disposal
            </a>
            
            <a href="{{ route('stock_eng.transaction.disposal.scan') }}?mode=manual" 
               class="inline-flex items-center justify-center gap-1.5 h-9 rounded-lg bg-gradient-to-r from-red-600 via-rose-600 to-pink-600 px-3.5 text-[11px] font-black text-white shadow-md hover:opacity-90 tracking-wider uppercase active:scale-95 transition-all font-nunito w-full sm:w-auto text-center no-underline">
                <i class="fa-solid fa-keyboard text-xs"></i> Manual Disposal
            </a>
        </div>
    </div>
    ...
    <div class="w-full overflow-hidden rounded-xl border border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900 pt-4 shadow-sm">
            ...
            <div class="grid grid-cols-12 gap-2 w-full sm:w-auto order-1 sm:order-2">
                ...
                <div class="relative col-span-8 sm:w-60 sm:block">
                    ...
                    <form action="{{ url()->current() }}" method="GET" class="w-full">
                        ...
                        @endif
                        <input type="text" name="search" value="{{ request('search') }}" id="tableSearch" placeholder="Search TX ID, Sparepart, NIK..." class="w-full rounded-lg border border-gray-300 dark:border-slate-700 bg-transparent py-2 pl-9 pr-3 text-xs md:text-[13px] outline-none focus:border-rose-500 text-slate-950 dark:text-white font-bold font-nunito">
                    </form>
                </div>

                {{-- TOMBOL EXPORT CSV --}}
                <button type="button" onclick="exportTableToCSV('stock-disposal-activities.csv')" class="col-span-4 flex items-center justify-center gap-1.5 rounded-lg border border-gray-300 dark:border-slate-700 bg-white dark:bg-slate-800 px-2 sm:px-3.5 py-2 text-xs md:text-[13px] font-black text-slate-950 dark:text-white shadow-sm hover:bg-slate-50 dark:hover:bg-slate-700 transition-all active:scale-95 cursor-pointer font-nunito">
        ...
        {{-- AREA SCROLL HORIZONTAL TABEL TEMATIC ROSE / RED --}}
        <div class="w-full overflow-x-auto scrollbar-thin bg-transparent">
            <table class="w-full table-fixed text-center border-collapse border-b border-gray-200 dark:border-slate-800 min-w-[1700px]" id="historyTable">
                <thead>
                    <tr class="text-[12px] font-black uppercase tracking-wider bg-rose-600 dark:bg-rose-950/80 text-white dark:text-rose-200 font-nunito table-header-row">
                        <th class="px-2 py-3.5 w-[50px] text-center">
                            <input type="checkbox" id="selectAllCheckbox" class="w-4 h-4 rounded border-rose-400 bg-transparent text-rose-600 focus:ring-rose-500 cursor-pointer checked:bg-white checked:border-white">
                        </th>
                        <th class="px-2 py-3.5 w-[60px] border-l border-rose-500 bg-rose-700/30">NO</th>
                        <th class="px-3 py-3.5 w-[200px] border-l border-rose-500 bg-rose-700/30">TRANSACTION DISPOSAL ID</th>
                        <th class="px-3 py-3.5 w-[170px] border-l border-rose-500 bg-rose-700/30">OPERATOR</th>
                        <th class="px-3 py-3.5 w-[170px] border-l border-rose-500 bg-rose-700/30">DISPOSAL NO / REASON</th>
                        <th class="px-3 py-3.5 w-[180px] border-l border-rose-500 bg-rose-700/30">BARCODE ID</th>
                        <th class="px-3 py-3.5 w-[150px] border-l border-rose-500 bg-rose-700/30">SPAREPART ID</th>
                        <th class="px-2 py-3.5 w-[110px] border-l border-rose-500 bg-rose-700/30">RAK ID</th>
                        <th class="px-2 py-3.5 w-[110px] border-l border-rose-500 bg-rose-700/30">QTY DISPOSAL</th>
                        <th class="px-3 py-3.5 w-[120px] border-l border-rose-500 bg-rose-700/30">STATUS</th>
                        <th class="px-3 py-3.5 w-[140px] border-l border-rose-500 bg-rose-700/30">PROCESS TYPE</th>
                        <th class="px-4 py-3.5 w-[220px] border-l border-rose-500 bg-rose-700/30 text-left">REMARK AUDIT</th>
                        <th class="px-3 py-3.5 w-[150px] border-l border-rose-500 bg-rose-700/30 text-center">TIMESTAMP</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-200 dark:divide-slate-800 text-[13px] font-bold font-nunito bg-transparent table-body-data">
                    ...
                    <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-850/40 transition-colors duration-150 bg-transparent">
                        <td class="px-2 py-3.5 text-center">
                            <input type="checkbox" class="row-checkbox w-4 h-4 rounded border-gray-300 dark:border-slate-700 text-rose-600 focus:ring-rose-500 cursor-pointer">
                        </td>
                        ...
                        {{-- 2. TRANSACTION DISPOSAL ID --}}
                        <td class="px-3 py-3.5 border-l border-gray-100 dark:border-slate-800 font-extrabold font-mono text-rose-600 dark:text-rose-400 text-center whitespace-nowrap select-all">
                            {{ $log->tx_id ?? $log->transaction_disposal_id ?? '-' }}
                        </td>
                        ...
                        {{-- 4. DISPOSAL NO / REASON --}}
                        <td class="px-3 py-3.5 border-l border-gray-100 dark:border-slate-800 font-extrabold tracking-wide text-center whitespace-nowrap text-rose-600 dark:text-rose-400">
                            {{ $log->disposal_no ?? $log->disposal_req_no ?? $log->reason ?? '-' }}
                        </td>
                        ...
                        <td class="px-3 py-3.5 border-l border-gray-100 dark:border-slate-800 font-mono text-emerald-600 dark:text-emerald-400 text-center whitespace-nowrap">
                            {{ $log->stockEng->sparepart->sparepart_id ?? $log->stockEng->part_no ?? $log->sparepart_id ?? '-' }}
                        </td>
                        ...
                        <td class="px-2 py-3.5 border-l border-gray-100 dark:border-slate-800 text-center whitespace-nowrap">
                            <span class="inline-flex items-center justify-center rounded-lg px-2.5 py-0.5 text-[11px] font-black border border-rose-200 bg-rose-50 text-rose-600 dark:bg-rose-500/10 dark:text-rose-400 dark:border-rose-900/50">
                                -{{ number_format($log->qty_transaction ?? $log->qty ?? 0) }} Pcs
                            </span>
                        </td>
                        ...
                        <td class="px-3 py-3.5 border-l border-gray-100 dark:border-slate-800 text-center whitespace-nowrap">
                            ...
                            <span class="inline-flex items-center justify-center rounded-lg px-2.5 py-0.5 text-[10px] font-black tracking-tight uppercase border
                                @if($isProcManual) border-amber-200 bg-amber-50 text-amber-700 dark:bg-amber-500/10 dark:text-amber-400 dark:border-amber-900/50
                                @else border-teal-200 bg-teal-50 text-teal-700 dark:bg-teal-500/10 dark:text-teal-400 dark:border-teal-900/50 @endif">
                                {{ $log->process_type ?? 'Scan' }} Disposal
                            </span>
                        </td>
