...
<div class="font-nunito w-full p-3 md:p-6 bg-slate-50/30 dark:bg-slate-950 min-h-screen transition-all duration-300">
    ...
    <div class="w-full overflow-hidden rounded-xl border border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900 pt-4 shadow-sm">
            ...
            <div class="grid grid-cols-12 gap-2 w-full sm:w-auto order-1 sm:order-2">
                ...
                <div class="relative col-span-8 sm:w-64 sm:block">
                    ...
                    <form action="{{ url()->current() }}" method="GET" class="w-full">
                        ...
                        @endif
                        <input type="text" name="search" value="{{ request('search') }}" id="tableSearch" placeholder="Search TX Disposal, Barcode, NIK..." class="w-full rounded-lg border border-gray-300 dark:border-slate-700 bg-transparent py-2 pl-9 pr-3 text-xs md:text-[13px] outline-none focus:border-rose-500 text-slate-950 dark:text-white font-bold font-nunito">
                    </form>
                </div>

                {{-- TOMBOL EXPORT CSV --}}
                <button type="button" onclick="exportTableToCSV('stock-disposal-activities.csv')" class="col-span-4 flex items-center justify-center gap-1.5 rounded-lg border border-gray-300 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-2 text-xs md:text-[13px] font-black text-slate-950 dark:text-white shadow-sm hover:bg-slate-50 dark:hover:bg-slate-700 transition-all active:scale-95 cursor-pointer font-nunito">
        ...
        {{-- AREA SCROLL HORIZONTAL TABEL MERAH DISPOSAL (TANPA RAK) --}}
        <div class="w-full overflow-x-auto scrollbar-thin bg-transparent">
            <table class="w-full table-fixed text-center border-collapse border-b border-gray-200 dark:border-slate-800 min-w-[1800px]" id="historyTable">
                <thead>
                    <tr class="text-[12px] font-black uppercase tracking-wider bg-rose-600 dark:bg-rose-900 text-white font-nunito table-header-row">
                        ...
                        <th class="px-4 py-4 w-[160px] border-l border-rose-500/50 bg-rose-700/50 text-center">UPDATED AT</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-200 dark:divide-slate-800 text-[13px] font-bold font-nunito bg-transparent table-body-data">
                    ...
                    <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-850/40 transition-colors duration-150 bg-transparent">
                        ...
                        <td class="px-5 py-4 border-l border-gray-100 dark:border-slate-800 font-extrabold font-mono text-rose-600 dark:text-rose-400 text-center whitespace-nowrap select-all">
                            {{ $log->tx_id ?? '-' }}
                        </td>
                        ...
                        <td class="px-5 py-4 border-l border-gray-100 dark:border-slate-800 text-center whitespace-nowrap font-extrabold text-slate-900 dark:text-white">
                            {{ $log->user->name ?? $log->operator_name ?? 'Unknown' }}
                        </td>
                        ...
                        <td class="px-4 py-4 border-l border-gray-100 dark:border-slate-800 font-mono text-emerald-600 dark:text-emerald-400 text-center font-extrabold whitespace-nowrap">
                            {{ $log->stockEng->sparepart->sparepart_id ?? $log->stockEng->sparepart_id ?? $log->sparepart_id ?? '-' }}
                        </td>
