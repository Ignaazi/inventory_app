    ...
    <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white p-6 md:p-8 dark:border-gray-800 dark:bg-white/[0.03]">
        ...
        <div class="flex items-center justify-between mb-8 pb-5 border-b border-gray-100 dark:border-gray-800">
            <div>
                ...
                <p class="text-xs text-slate-500">Pilih item Nozzle, lokasi Rak asal, pasangkan dengan Barcode ID. Data Production Request akan terisi otomatis.</p>
            </div>
        ...
        <form action="{{ route('eng.out.store') }}" method="POST" id="form-manual-out">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
                ...
                {{-- 1. DROPDOWN BARCODE (Dinaikkan ke atas agar menjadi fokus utama input) --}}
                <div class="flex flex-col gap-2">
                    <label class="text-xs font-bold text-slate-700 dark:text-gray-300">Barcode ID Label <span class="text-amber-500">* Once Used</span></label>
                    <div class="relative">
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <input type="number" name="qty_out" value="{{ old('qty_out') }}" required min="1" 
                           class="w-full bg-white dark:bg-transparent border border-gray-300 dark:border-gray-700 rounded-lg px-4 py-2.5 text-sm font-medium text-slate-950 dark:text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500" placeholder="Masukkan jumlah qty...">
                </div>

                {{-- 5. COMMENT --}}
                <div class="flex flex-col gap-2 md:col-span-2">
                    <label class="text-xs font-bold text-slate-700 dark:text-gray-300">Comment / Note</label>
                    <textarea name="comment" rows="2" class="w-full bg-white dark:bg-transparent border border-gray-300 dark:border-gray-700 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500" placeholder="Catatan tambahan transaksi out...">{{ old('comment') }}</textarea>
                </div>
            </div>

            <div class="mt-8 pt-5 border-t border-gray-100 dark:border-gray-800 flex justify-end">
                <button type="button" onclick="document.getElementById('form-manual-out').submit();" class="bg-gradient-to-r from-blue-600 via-blue-700 to-amber-500 text-white px-6 py-2.5 rounded-lg text-sm font-bold shadow-md hover:opacity-90 transition-opacity">
                    Submit Out
                </button>
            </div>
        </form>
