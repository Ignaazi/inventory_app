...
<div class="font-nunito w-full p-3 md:p-6 bg-slate-50/30 dark:bg-slate-950 min-h-screen transition-all duration-300">
    ...
    <div class="mb-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 font-nunito">
        <div>
            <h2 class="text-xl md:text-2xl font-black text-slate-900 dark:text-white tracking-tight uppercase">Stock Return Activities</h2>
            <p class="text-[11px] md:text-[13px] font-bold text-slate-500 dark:text-slate-400">Track and monitor your recent sparepart return logs and restock history</p>
        </div>

        {{-- Tombol Tunggal Scan Return (Warna Orange) --}}
        <div class="flex items-center gap-2 w-full sm:w-auto">
            <a href="{{ route('eng.return.scan') }}" 
               class="inline-flex items-center justify-center gap-2 h-10 rounded-xl bg-gradient-to-r from-orange-500 via-amber-500 to-orange-600 px-5 text-xs font-black text-white shadow-md shadow-orange-500/20 hover:shadow-lg hover:brightness-110 tracking-wider uppercase active:scale-95 transition-all font-nunito w-full sm:w-auto text-center no-underline">
                <i class="fa-solid fa-rotate-left text-sm"></i> Scan Return
            </a>
        </div>
    </div>
    ...
    <div class="w-full overflow-hidden rounded-xl border border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900 pt-4 shadow-sm">
            ...
            <div class="grid grid-cols-12 gap-2 w-full sm:w-auto order-1 sm:order-2">
                ...
                <div class="relative col-span-8 sm:w-64 sm:block">
                    ...
                    <form action="{{ url()->current() }}" method="GET" class="w-full">
                        ...
                        @endif
                        <input type="text" name="search" value="{{ request('search') }}" id="tableSearch" placeholder="Search TX Return, Barcode, NIK..." class="w-full rounded-lg border border-gray-300 dark:border-slate-700 bg-transparent py-2 pl-9 pr-3 text-xs md:text-[13px] outline-none focus:border-indigo-500 text-slate-950 dark:text-white font-bold font-nunito">
                    </form>
                </div>

                {{-- TOMBOL EXPORT CSV --}}
                <button type="button" onclick="exportTableToCSV('stock-return-activities.csv')" class="col-span-4 flex items-center justify-center gap-1.5 rounded-lg border border-gray-300 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-2 text-xs md:text-[13px] font-black text-slate-950 dark:text-white shadow-sm hover:bg-slate-50 dark:hover:bg-slate-700 transition-all active:scale-95 cursor-pointer font-nunito">
        ...
        {{-- AREA SCROLL HORIZONTAL TABEL BIRU DENGAN SPASING LONGGAR --}}
                ...
                <thead>
                    <tr class="text-[12px] font-black uppercase tracking-wider bg-blue-600 dark:bg-blue-900 text-white font-nunito table-header-row">
                        <th class="px-3 py-4 w-[50px] text-center">
                            <input type="checkbox" id="selectAllCheckbox" class="w-4 h-4 rounded border-blue-400 bg-transparent text-indigo-600 focus:ring-indigo-500 cursor-pointer checked:bg-white checked:border-white">
                        </th>
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-slate-800 text-[13px] font-bold font-nunito bg-transparent table-body-data">
                    ...
                    <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-850/40 transition-colors duration-150 bg-transparent">
                        <td class="px-3 py-4 text-center">
                            <input type="checkbox" class="row-checkbox w-4 h-4 rounded border-gray-300 dark:border-slate-700 text-indigo-600 focus:ring-indigo-500 cursor-pointer">
                        </td>
                        ...
                        <td class="px-5 py-4 border-l border-gray-100 dark:border-slate-800 text-center whitespace-nowrap font-extrabold text-slate-900 dark:text-white">
                            {{ $log->name ?? $log->operator_name ?? 'Unknown' }}
                        </td>
                        ...
                        <td class="px-5 py-4 border-l border-gray-100 dark:border-slate-800 font-mono text-amber-600 dark:text-amber-400 text-center whitespace-nowrap">
                            {{ $log->barcode_id ?? $log->barcode_code ?? '-' }}
                        </td>
                        ...
                        <td class="px-4 py-4 border-l border-gray-100 dark:border-slate-800 font-mono text-emerald-600 dark:text-emerald-400 text-center font-extrabold whitespace-nowrap">
                            {{ $log->sparepart_id ?? $log->part_name ?? '-' }}
                        </td>
