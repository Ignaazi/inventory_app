...
<div class="stock-return-view mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
  ...
  <div class="flex flex-col gap-4 mb-6 sm:flex-row sm:items-center sm:justify-between">
    <div>
      ...
      <p class="text-xs font-semibold text-slate-500 dark:text-gray-400 mt-0.5">Track your recent sparepart return and rollback activities</p>
    </div>
    ...
    <div class="flex flex-col gap-4 mb-4 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div class="flex flex-wrap items-center gap-3">
        <div class="inline-flex p-1 bg-gray-100 dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-inner">
          ...
          <button type="button" onclick="filterTable('manual return', this)" class="filter-btn px-3 py-1 text-xs font-extrabold rounded-lg transition-all duration-200 text-slate-500 dark:text-gray-400 hover:text-slate-950 dark:hover:text-white">
            Manual Return
          </button>
          <button type="button" onclick="filterTable('scan return', this)" class="filter-btn px-3 py-1 text-xs font-extrabold rounded-lg transition-all duration-200 text-slate-500 dark:text-gray-400 hover:text-slate-950 dark:hover:text-white">
            Scan Return
          </button>
        </div>
      ...
      <table class="min-w-full text-left border-collapse" id="history-table">
        <thead>
          <tr class="border-gray-200 border-y dark:border-gray-800 bg-gray-50/70 dark:bg-slate-800/60">
            ...
            <th class="py-2.5 px-3 text-[10px] font-black text-slate-950 uppercase tracking-wider dark:text-white text-center">RETURN ID</th>
            ...
            <th class="py-2.5 px-3 text-[10px] font-black text-slate-950 uppercase tracking-wider dark:text-white text-center">QTY RETURN</th>
        ...
        <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
          ...
          <tr class="table-row-item hover:bg-gray-50/60 transition-colors duration-200 dark:hover:bg-white/[0.02]">
            ...
            <td class="py-2 px-3 text-[11px] font-bold text-slate-950 dark:text-white text-center font-mono tracking-tight whitespace-nowrap">
              {{ $log->dbBarcode->barcode_id ?? ($log->barcode_id ?? '-') }}
            </td>
            <td class="py-2 px-3 text-[11px] font-bold text-slate-950 dark:text-white text-center font-mono tracking-tight whitespace-nowrap">
          ...
          <tr>
            <td colspan="13" class="text-center py-6 text-xs font-bold text-slate-400 uppercase tracking-wider">
              No recent stock return logs found.
            </td>
          </tr>
          @endforelse
        </tbody>
...
<script>
  function filterTable(criteria, element) {
    ...
    rows.forEach(row => {
      ...
      } else if (criteria === 'manual return') {
        if (remarkText.includes('manual')) {
          row.style.display = '';
        } else {
          row.style.display = 'none';
        }
      } else if (criteria === 'scan return') {
