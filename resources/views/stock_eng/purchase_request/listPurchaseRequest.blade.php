...
<div class="font-nunito w-full p-3 md:p-6 bg-slate-50/30 dark:bg-slate-950 min-h-screen transition-all duration-300">
    ...
    <div class="mb-4 flex items-center gap-2 rounded-xl border border-emerald-200 bg-emerald-50 dark:bg-emerald-950/30 dark:border-emerald-900/50 px-3 py-2.5 md:px-4 md:py-3 shadow-sm">
        ...
        <p class="text-[12px] md:text-[14px] font-bold text-emerald-800 dark:text-emerald-400 font-nunito leading-tight">
            ...
            Total {{ $requests->total() }} purchase requests logged for approval routing.
        </p>
    </div>
    ...
    <div class="mb-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 font-nunito">
        <div>
            <h2 class="text-xl md:text-2xl font-black text-slate-900 dark:text-white tracking-tight">Purchase Request Verification List</h2>
            <p class="text-[11px] md:text-[13px] font-bold text-slate-500 dark:text-slate-400">PT SIIX EMS KARAWANG — ENGINEERING STOCK</p>
        </div>
    </div>
    ...
    <div class="w-full overflow-hidden rounded-xl border border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900 pt-4 shadow-sm">
            ...
            <div class="grid grid-cols-12 gap-2 w-full sm:w-auto order-1 sm:order-2">
                ...
                <div class="relative col-span-8 sm:w-60 sm:block">
                    ...
                    <form action="{{ url()->current() }}" method="GET" class="w-full">
                        <input type="text" name="search" value="{{ request('search') }}" id="tableSearch" placeholder="Search PR Code / User..." class="w-full rounded-lg border border-gray-300 dark:border-slate-700 bg-transparent py-2 pl-9 pr-3 text-xs md:text-[13px] outline-none focus:border-blue-500 text-slate-950 dark:text-white font-bold font-nunito">
                    </form>
                </div>

                {{-- TOMBOL EXPORT CSV --}}
                <button type="button" onclick="exportTableToCSV('purchase-requests.csv')" class="col-span-4 flex items-center justify-center gap-1.5 rounded-lg border border-gray-300 dark:border-slate-700 bg-white dark:bg-slate-800 px-2 sm:px-3.5 py-2 text-xs md:text-[13px] font-black text-slate-950 dark:text-white shadow-sm hover:bg-slate-50 dark:hover:bg-slate-700 transition-all active:scale-95 cursor-pointer font-nunito">
        ...
        {{-- AREA SCROLL HORIZONTAL (UKURAN DINAIKKAN KE MIN-W 2020PX AGAR TIDAK SESAK) --}}
        <div class="w-full overflow-x-auto scrollbar-thin bg-transparent">
            <table class="w-full table-fixed text-center border-collapse border-b border-gray-200 dark:border-slate-800 min-w-[2020px]" id="approvalTable">
                <thead>
                    <tr class="text-[12px] font-black uppercase tracking-wider bg-orange-600 dark:bg-orange-950/80 text-white dark:text-orange-200 font-nunito table-header-row">
                        ...
                        <th class="px-3 py-3.5 w-[140px] border-l border-orange-500 bg-orange-700/30">Requester</th>
                        
                        {{-- DATA MATERIAL EXPANSION SECTIONS --}}
                        <th class="px-4 py-3.5 border-l border-orange-500 bg-orange-700/30 text-center w-[120px]">Sparepart ID</th>
                        <th class="px-4 py-3.5 border-l border-orange-500 bg-orange-700/30 text-center w-[140px]">Part Number</th>
                        <th class="px-4 py-3.5 border-l border-orange-500 bg-orange-700/30 text-center w-[130px]">SAP Code</th>
                        <th class="px-4 py-3.5 border-l border-orange-500 bg-orange-700/30 text-center w-[130px]">Category</th>
                        
                        <th class="px-2 py-3.5 w-[80px] border-l border-orange-500 bg-orange-700/30">QTY PR</th>
                        ...
                        <th class="px-4 py-3.5 border-l border-orange-500 bg-orange-700/30 text-center w-[130px]">Remark </th>
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-slate-800 text-[13px] font-bold font-nunito bg-transparent table-body-data">
                    ...
                    @php
                        ...
                        } elseif(str_contains($rawStatus, 'approved') || str_contains($rawStatus, 'success')) {
                            $statusText = 'approved';
                    ...
                    <tr class="table-row-item hover:bg-slate-50/50 dark:hover:bg-slate-850/40 transition-colors duration-150 bg-transparent">
                        ...
                        <td class="px-3 py-3.5 border-l border-gray-100 dark:border-slate-800 whitespace-nowrap">
                            {{ optional($req->user)->nim ?? optional($req->user)->nik ?? '-' }}
                        </td>
                        ...
                        {{-- ACTUAL SPAREPART DATA MAPPING (FIXED: MENAMPILKAN KODE STRING BUKAN PRIMARY KEY ID) --}}
                        <td class="px-4 py-3.5 text-center border-l border-gray-100 dark:border-slate-800 font-extrabold tracking-wide whitespace-normal break-words leading-normal text-indigo-600 dark:text-indigo-400">
                            {{ optional($req->sparepart)->sparepart_id ?? '-' }}
                        </td>

                        <td class="px-4 py-3.5 text-center border-l border-gray-100 dark:border-slate-800 tracking-wide whitespace-normal break-words leading-normal">
                            {{ optional($req->sparepart)->part_number ?? '-' }}
                        </td>

                        <td class="px-4 py-3.5 text-center border-l border-gray-100 dark:border-slate-800 tracking-wide whitespace-normal break-words leading-normal">
                            {{ optional($req->sparepart)->sap_code ?? '-' }}
                        </td>

                        <td class="px-4 py-3.5 text-center border-l border-gray-100 dark:border-slate-800 tracking-wide whitespace-normal break-words leading-normal uppercase text-[11px]">
                            {{ optional($req->sparepart)->category ?? '-' }}
                        </td>
                        ...
                        <td class="px-2 py-3.5 border-l border-gray-100 dark:border-slate-800 uppercase whitespace-nowrap">
                            <span class="{{ $req->priority == 'urgent' ? 'text-rose-600 font-black animate-pulse' : 'text-slate-600' }}">
                                {{ $req->priority }}
                            </span>
                        </td>
                        ...
                        <td class="px-3 py-3.5 border-l border-gray-100 dark:border-slate-800 font-semibold whitespace-nowrap text-center">
                            ...
                            <div class="text-[10px] mt-0.5 text-slate-500">{{ $req->created_at ? $req->created_at->format('H:i') : '-' }} WIB</div>
                        </td>

                        {{-- NEW TIMESTAMPS: UPDATED AT --}}
                        <td class="px-3 py-3.5 border-l border-gray-100 dark:border-slate-800 font-semibold whitespace-nowrap text-center">
                            <div class="font-bold text-slate-800 dark:text-slate-200">{{ $req->updated_at ? $req->updated_at->format('d/m/Y') : '-' }}</div>
                            <div class="text-[10px] mt-0.5 text-slate-500">{{ $req->updated_at ? $req->updated_at->format('H:i') : '-' }} WIB</div>
                        </td>
                        
                        {{-- DECISION ACTION WORKFLOW (REJECT & CHECKED) --}}
                        <td class="px-4 py-3.5 border-l border-gray-100 dark:border-slate-800 text-center">
                            <div class="flex items-center justify-center gap-1.5 w-full">
                                @if(strtolower($req->status) == 'pending')
                                    {{-- FORM ACTION REJECT --}}
                                    <form action="{{ route('purchase.request.reject', $req->id) }}" method="POST" class="reject-form inline-block m-0">
                                        @csrf
                                    ...
                                    {{-- NEW LINK DIRECT TO BLADE CHECKED VIEW --}}
                                    <a href="{{ route('purchase.request.checked.view', $req->id) }}" class="inline-block text-center px-2.5 py-1.5 bg-gradient-to-r from-emerald-500 to-blue-500 text-white font-black rounded text-[10px] uppercase tracking-wider shadow-sm hover:opacity-90 active:scale-95 transition-all cursor-pointer">
                                        Checked
                                    </a>
                                @else
                    ...
                    <tr>
                        <td colspan="17" class="py-10 text-center italic font-medium text-[13px] font-nunito dark:bg-slate-900 table-empty-text">
                            No purchase requests waiting in verifier queue.
                        </td>
                    </tr>
                    @endforelse
                </tbody>
    ...
    document.querySelectorAll('.reject-form').forEach(form => {
        form.addEventListener('submit', function(e) {
            ...
            Swal.fire({
                ...
                text: "Apakah Anda yakin ingin menolak pengajuan dokumen material ini?",
                icon: 'warning',
